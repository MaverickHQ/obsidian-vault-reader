...
Automated review flagged release/source remediation items on 2026-07-04. Version `0.1.2` is the corrected review candidate.
...
The release tag must match `manifest.json` exactly. `manifest.json`, `package.json`, and `versions.json` must agree on `0.1.2`, and `versions.json` must map `0.1.2` to the release `minAppVersion`.

## Pre-Submission Gates

- Production-facing repository is `MaverickHQ/obsidian-vault-reader`.
- Repository visibility is public.
- GitHub Release `0.1.2` is published from the tag-based GitHub release workflow.
...
- Release tag matches manifest version `0.1.2`.
