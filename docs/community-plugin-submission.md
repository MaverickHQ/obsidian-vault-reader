...
Automated review passed for `0.1.3` on 2026-07-04. Version `0.1.4` is the current highlight reliability release candidate.
...
The release tag must match `manifest.json` exactly. `manifest.json`, `package.json`, and `versions.json` must agree on `0.1.4`, and `versions.json` must map `0.1.4` to the release `minAppVersion`.

## Pre-Submission Gates

- Production-facing repository is `MaverickHQ/obsidian-vault-reader`.
- Repository visibility is public.
- GitHub Release `0.1.4` is published from the tag-based GitHub release workflow.
...
- Release tag matches manifest version `0.1.4`.
