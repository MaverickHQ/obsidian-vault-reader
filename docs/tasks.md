...
- [ ] Create a new public GitHub repo for the clean project, or create an orphan public branch only if a new repo is deliberately rejected.
- [ ] Initialize the public repo from the clean tree with a single first commit such as `Initial public release candidate`.
- [ ] Set public repo topics: `obsidian-plugin`, `speed-reading`, `rsvp`, `markdown`, `reading`, `typescript`.
- [ ] Confirm public repo settings: Issues enabled, Dependabot enabled, private vulnerability reporting enabled when available, branch protection planned for `main`.
- [ ] Confirm public repo description is concise and searchable without relying on the term `RSVP`.
- [ ] [T] RED: add a public-repo-readiness checklist/policy that fails until repo URL, visibility decision, topics, description, issue settings, and security settings are documented.
- [ ] [T] GREEN: document the public repo setup evidence.
- [ ] [T] REFACTOR: keep public repo setup docs separate from Obsidian submission docs.
...
- [ ] Build release assets from the clean public tree.
- [ ] Verify GitHub Release tag will match `manifest.json` version exactly.
- [ ] Verify release assets include `main.js`, `manifest.json`, `styles.css`, and checksum evidence.
- [ ] Verify Obsidian install behavior expectations match the Obsidian release contract: repo `manifest.json` and `README.md` are public, release tag matches manifest version, and installable assets are attached to the release.
- [ ] [T] RED: add release-submission policy tests before creating public release assets.
- [ ] [T] GREEN: generate release assets from the clean public tree and pass the tests.
- [ ] [T] REFACTOR: share release asset validation with Phase 3.5 submission checks.
...
- [ ] Run history-aware secret scan before public visibility is enabled.
- [ ] Confirm the clean public repo is ready for Phase 3.5 submission packaging.
- [ ] [T] Phase-level test gate: clean public repo preparation must be covered by policy tests plus manual evidence for GitHub visibility/settings.
...
- [ ] Create or verify a GitHub Release whose tag exactly matches `manifest.json` version.
- [ ] Attach `main.js`, `manifest.json`, and `styles.css` to the GitHub Release.
- [ ] Verify `versions.json` includes the release version and correct `minAppVersion`.
- [ ] Prepare the `obsidianmd/obsidian-releases` PR entry for `community-plugins.json`.
- [ ] Use searchable listing copy that explains the product without relying on the term `RSVP`.
- [ ] Verify a clean vault can install from the release package before submission.
- [ ] Optionally run a short BRAT/public beta before submitting if manual QA finds uncertainty or if we want early social proof.
- [ ] [T] RED: add failing release-submission policy tests for tag/version alignment, release assets, and community listing metadata.
- [ ] [T] GREEN: generate the release package and submission metadata.
- [ ] [T] REFACTOR: keep submission metadata in docs so future releases are repeatable.
